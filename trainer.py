...
class Trainer:
    def __init__(self, options):
        ...
        train_dataset = self.dataset(
            ...
            self.opt.frame_ids, num_scales, is_train=True, img_ext=img_ext)
        self.train_loader = DataLoader(
            train_dataset, self.opt.batch_size, True,
            num_workers=self.opt.num_workers, pin_memory=True, drop_last=True)
        val_dataset = self.dataset(
            self.opt.data_path, val_filenames, self.opt.height, self.opt.width,
            self.opt.frame_ids, num_scales, is_train=False, img_ext=img_ext)
    ...
    def run_epoch(self):
        ...
        for batch_idx, inputs in enumerate(self.train_loader):
            ...
            if self.step % 250 == 0:
                self.log_time(batch_idx, duration, losses["loss"].cpu().data, data_loading_time, gpu_time)
    ...
    def val(self):
        ...
        with torch.no_grad():
            ...
            print('Validation Loss: {:.5f}'.format(losses['loss'].cpu().data))
    ...
    def compute_depth_losses(self, inputs, outputs, losses):
        ...
        depth_pred = torch.clamp(F.interpolate(
            depth_pred, [orig_h, orig_w], mode="bilinear", align_corners=False), self.opt.min_depth, self.opt.max_depth)
    ...
    def log_time(self, batch_idx, duration, loss, data_time, gpu_time):
        ...
        print(print_string.format(self.epoch, batch_idx, samples_per_sec, loss,
                                  sec_to_hm_str(time_sofar), sec_to_hm_str(training_time_left), data_time, gpu_time))
